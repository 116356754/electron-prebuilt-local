...
```
npm install erayt-ele-prebuilt --save-dev
```
...
```
npm install -g erayt-ele-prebuilt
```

If that command fails with an `EACCESS` error you may have to run it again with `sudo`:

```
sudo npm install -g erayt-ele-prebuilt
```
...
``` js
var electron = require('erayt-ele-prebuilt')
